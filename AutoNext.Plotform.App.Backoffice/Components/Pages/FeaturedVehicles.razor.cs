...
namespace AutoNext.Plotform.App.Backoffice.Components.Pages
{
    public class FeaturedVehiclesBase:ComponentBase
    {
        ...
        [Inject] protected ILogger<FeaturedVehiclesBase> Logger { get; set; } = default!;
        ...
        // Data
        protected List<FeaturedVehicleResponseDto> AllVehicles { get; set; } = new();
        protected List<FeaturedVehicleResponseDto> SelectedVehicles { get; set; } = new();
        ...
        protected FeaturedVehicleResponseDto? SelectedVehicle { get; set; } = null;
        protected FeaturedVehicleResponseDto? ActivateVehicleModel { get; set; } = null;
        protected DateTime? ActivateEndDate { get; set; }
        protected int BulkPriorityValue { get; set; } = 5;
        protected FeaturedVehicleRequestDto FormModel { get; set; } = new();
        ...
        protected List<FeaturedVehicleResponseDto> FilteredVehicles => AllVehicles
            ...
            .OrderByDescending(v => SortBy switch
            {
        ...
        protected List<FeaturedVehicleResponseDto> PaginatedVehicles => FilteredVehicles
        ...
        {
            Logger.LogInformation("FeaturedVehicles page initialized");
        ...
        protected async Task LoadVehiclesAsync()
        {
            try
            {
                ...
                var result = await FeaturedVehicleService.GetAllAsync(1, 1000);
                AllVehicles = result?.Items?.ToList() ?? new List<FeaturedVehicleResponseDto>();
                TotalCount = AllVehicles.Count;
                CurrentPage = 1;
                SelectedVehicles.Clear();

                Logger.LogInformation("Loaded {Count} featured vehicles", AllVehicles.Count);
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "Error loading featured vehicles");
                NotificationService.Notify(NotificationSeverity.Error, "Error", "Failed to load featured vehicles.");
            }
            finally
            {
                IsLoading = false;
        ...
        protected void OpenAddModal()
        {
            ...
            FormModel = new FeaturedVehicleRequestDto
            {
                StartDate = DateTime.UtcNow,
                EndDate = DateTime.UtcNow.AddDays(30),
                IsActive = true,
                Priority = 5,
                Price = new PriceInfoDto(),
                Location = new LocationInfoDto(),
                Seller = new SellerInfoDto(),
                KeySpecifications = new KeySpecificationsDto(),
                TestDrive = new TestDriveInfoDto()
            };
            ShowAddEditModal = true;
        }

        protected void OpenEditModal(FeaturedVehicleResponseDto vehicle)
        {
            //IsEditMode = true;
            ...
            Navigation.NavigateTo($"/featured-vehicles/{vehicle.Id}/edit");
        }
        ...
        protected async Task SaveVehicle()
        {
            try
            {
                ...
                {
                    var updated = await FeaturedVehicleService.UpdateAsync(SelectedVehicle.Id, FormModel);
                    if (updated != null)
                    {
                ...
                {
                    var created = await FeaturedVehicleService.CreateAsync(FormModel);
                    NotificationService.Notify(NotificationSeverity.Success, "Created",
                        $"{FormModel.BrandName} {FormModel.ModelName} added as featured vehicle");
                }
            ...
            {
                Logger.LogError(ex, "Error saving featured vehicle");
                NotificationService.Notify(NotificationSeverity.Error, "Error", $"Failed to save: {ex.Message}");
            }
        }

        protected async Task ActivateVehicle(FeaturedVehicleResponseDto vehicle)
        {
        ...
        protected async Task ConfirmActivate()
        {
            ...
            try
            {
                var result = await FeaturedVehicleService.ActivateAsync(ActivateVehicleModel.Id, ActivateEndDate);
                if (result)
                {
        ...
        protected async Task DeactivateVehicle(FeaturedVehicleResponseDto vehicle)
        {
            try
            {
                var result = await FeaturedVehicleService.DeactivateAsync(vehicle.Id);
                if (result)
                {
        ...
        protected async Task DeleteVehicle(FeaturedVehicleResponseDto vehicle)
        {
            var confirmed = await JSRuntime.InvokeAsync<bool>("confirm",
                $"Delete featured vehicle '{vehicle.BrandName} {vehicle.ModelName}'? This cannot be undone.");
            if (!confirmed) return;

            try
            {
                var result = await FeaturedVehicleService.DeleteAsync(vehicle.Id);
                if (result)
                {
        ...
        protected async Task BulkDelete()
        {
            ...
            var confirmed = await JSRuntime.InvokeAsync<bool>("confirm",
                $"Delete {SelectedVehicles.Count} featured vehicles? This cannot be undone.");
            if (!confirmed) return;
            ...
            try
            {
                foreach (var vehicle in SelectedVehicles.ToList())
                {
                    try
                    {
                        var success = await FeaturedVehicleService.DeleteAsync(vehicle.Id);
        ...
        protected async Task ExecuteBulkPriorityUpdate()
        {
            ...
            try
            {
                var result = await FeaturedVehicleService.BulkUpdatePriorityAsync(priorityUpdates);
                if (result)
                {
                    NotificationService.Notify(NotificationSeverity.Success, "Updated",
        ...
        protected void OnVehicleSelectionChanged(FeaturedVehicleResponseDto vehicle, bool isChecked)
        {
            if (isChecked)
            {
        ...
        {
            if (priority >= 8) return "#dc3545"; // Red - Highest
            if (priority >= 5) return "#fd7e14"; // Orange - High
            if (priority >= 3) return "#ffc107"; // Yellow - Medium
            return "#6c757d"; // Gray - Low
        }

        protected string GetThumbnail(FeaturedVehicleResponseDto vehicle)
        {
        ...
        {
            FormModel.Pros.Add(new ProConItemDto { Pro = string.Empty });
            StateHasChanged();
        }
        ...
        {
            FormModel.Cons.Add(new ProConItemDto { Con = string.Empty });
            StateHasChanged();
        }
